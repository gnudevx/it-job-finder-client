...
export const CreateJobProvider = ({ children, isEditing = false, jobId = null }) => {
  ...
  const updateField = (path, value) => {
    if (!path) return;
    setForm((prev) =>
      path.includes('.') ? setNestedPath(prev, path, value) : { ...prev, [path]: value }
    );
  };
  ...
  const handleSubmitJob = async (type = 'publish') => {
    ...
    if (!isEditing) {
      ...
      // Chỉnh sửa
      payload.publishStatus = initialPublishStatus === 'draft' ? 'draft' : 'pending';
    }
  ...
  useEffect(() => {
    ...
    const shouldSkip = Object.values(form).every(

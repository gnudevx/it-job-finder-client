  ...
  const handlePause = async (job) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setJobs((prev) =>
          prev.map((j) =>
            j._id === job._id ? { ...j, publishStatus: 'paused', visibility: 'hidden' } : j
          )
        );
      }
    } catch (err) {
  ...
  const handleRequestPublish = async (jobId) => {
    try {
      ...
      if (res.data.success) {
        ...
        setJobs((prev) =>
          prev.map((job) => (job._id === jobId ? { ...job, publishStatus: 'pending' } : job))
        );
      } else {
  ...
  const handleResume = async (job) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setJobs((prev) =>
          prev.map((j) =>
            j._id === job._id ? { ...j, publishStatus: 'approved', visibility: 'visible' } : j
          )
        );
      } else {
              ...
              <td>
                ...
                <div className={styles.tags}>
                  {job.visibility === 'visible' && (
                    <span className={styles.hidden}>Đang hiển thị</span>
                  )}
                  ...
                  {job.publishStatus === 'pending' && (
                    <span className={styles.pending}>Đang xét duyệt</span>
                  )}
                  {job.publishStatus === 'paused' && (
                    <span className={styles.pending}>Đã tạm dừng hiển thị</span>
                  )}
                  {job.publishStatus === 'approved' && (
                    <span className={styles.pending}>Đã duyệt</span>
                  )}
                  {job.publishStatus === 'rejected' && (
                    <span className={styles.pending}>Bị từ chối</span>
                  )}
                </div>
                ...
                <div className={styles.actions}>
                  ...
                  {job.publishStatus === 'draft' && (
                    <button
                      ...
                      onClick={() => handleRequestPublish(job._id)}
                    >
                      Yêu cầu hiển thị
                    </button>
                  )}
                  {job.publishStatus === 'paused' && (
                    <button className={styles.btnWhite} onClick={() => handleResume(job)}>
                      {' '}
                      hiển thị lại tin{' '}
                    </button>
                  )}
...
const calcRemainingDisplay = (job) => {
  if (job.publishStatus === 'pending') return 'Đang duyệt';

  if (job.publishStatus === 'rejected') return 'Bị từ chối';

  if (job.publishStatus === 'paused') return 'Đã tạm dừng hiển thị';

  if (job.publishStatus !== 'approved') return 'Tin chưa hiển thị';
